...
{context}"""
    ...
    async def generate_summary(self, project_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response = self.client.table("documents").select("id, filename").eq("project_id", project_id).eq("upload_status", "completed").execute()
