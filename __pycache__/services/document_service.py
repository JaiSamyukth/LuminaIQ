...
class DocumentService:
    ...
    async def delete_document(self, project_id: str, document_id: str):
        ...
        try:
            ...
            # 2. Delete chunks from DB (cascade should handle, but be explicit)
    ...
    async def process_chunks_from_db(
        self,
        document_id: str,
        project_id: str
    ):
        """
        Process pre-chunked document from database.
        Called by webhook when PDF service has finished chunking.
        Fetches chunks from document_chunks table, generates embeddings, and upserts to Qdrant.
        """
        try:
            # Update status to embedding
            await self._update_document_status(document_id, "embedding", "Generating embeddings...")
            ...
            if not response.data:
                await self._update_document_status(document_id, "failed", "No chunks found in database")
                return
            
            chunks_data = response.data
            chunks = [c["chunk_text"] for c in chunks_data]
            
            # 2. Get document filename for metadata
            doc_response = self.client.table("documents").select("filename").eq("id", document_id).single().execute()
            filename = doc_response.data.get("filename", "unknown") if doc_response.data else "unknown"
            
            logger.info(f"Processing {len(chunks)} chunks from database for document {filename}")
            
            # 3. Create Qdrant collection
            collection_name = f"project_{project_id}"
            await qdrant_service.create_collection(collection_name)
            
            # 4. Generate embeddings and Upsert (Batched)
            await self._update_document_status(document_id, "embedding", f"Generating embeddings ({len(chunks)} chunks)...")
            ...
            logger.error(f"Error processing chunks from DB: {str(e)}")
            await self._update_document_status(document_id, "failed", str(e))
